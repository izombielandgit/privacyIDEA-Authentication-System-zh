...
如果定义了otp_pin_random策略，您可以使用此策略定义随机PIN应该发生什么。动作的值取一个PinHandler的类，如privacyidea.lib.pinhandling.base.PinHandler。基础PinHandler只是将PIN记录到日志文件中。您可以添加类以通过EMail发送PIN或打印在纸上。有关更多信息，请参阅基类PinHandler。
