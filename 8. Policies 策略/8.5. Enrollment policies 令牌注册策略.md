...
令牌注册策略涉及域，客户端（请参阅Policies）和用户设置。
...
技术上，令牌注册策略控制REST API Token endpoints的使用，尤其是init和assign-methods。使用API Policies中的装饰器。
...
如果定义了otp_pin_random策略，您可以使用此策略定义随机PIN应该发生什么。操作的值取一个PinHandler的类，如privacyidea.lib.pinhandling.base.PinHandler。基础PinHandler只是将PIN记录到日志文件中。您可以添加类以通过EMail发送PIN或打印在纸上。有关更多信息，请参阅基类PinHandler。
...
如果管理员注册令牌或重置令牌的PIN，则该令牌的PIN被标记为在第一次（或下一次）使用时被改变。当用户使用旧PIN进行认证时，用户被成功认证。但是detail-response包含键“next_pin_change”和“pin_change”。 如果“pin_change”为True，则认证应用程序必须使用API/token/setpin触发PIN的更改。请参阅Token endpoints。
...
需要更改PIN的日期在/validate/check的API响应中返回。有关更多信息，请参阅change_pin_on_first_use。要指定PIN的内容，请参阅User Policies。
...
如果设置，令牌的OTP PIN将被加密。默认行为是OTP PIN的哈希值，这样更安全。
...
例：操作设置为lostTokenPWLen=10，lostTokenPWContents=Cns可以生成一个像`AC#!49MK))`这样的密码。
