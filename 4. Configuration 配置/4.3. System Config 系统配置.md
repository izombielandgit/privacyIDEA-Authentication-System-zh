...
Override Authorization client对于客户端特定策略（请参阅Policies）和RADIUS服务器或其他代理很重要。在RADIUS的情况下，privacyIDEA系统认证的客户端将始终是发出认证请求的RADIUS服务器。但是您可以允许RADIUS服务器IP发送另一个客户端信息（此处为RADIUS的客户端），以便为RADIUS的客户端评估策略。这样代理或RADIUS服务器可以用新的IP地址添加API参数client。此字段采用以逗号分隔映射到其他IP网络的IP网络列表。如：
