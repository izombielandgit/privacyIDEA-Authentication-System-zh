...
password属性是包含用户密码的数据库列。如果您对SQL数据库进行用户身份验证，则使用此方法。
...
要连接到SCIM服务，您需要提供到认证服务器的URL和到资源服务器的URL。认证服务器用于认证privacyIDEA服务器。身份验证基于client名称和此客户端的Secret。然后从资源服务器检索用户信息。属性映射的可用属性包括：
