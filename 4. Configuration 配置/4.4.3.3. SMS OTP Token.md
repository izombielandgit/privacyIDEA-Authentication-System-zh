...
sipgate provider连接到https://samurai.sipgate.net/RPC2，并且只有两个参数USERNAME和PASSWORD。参数必须在类似这样的字典中传递：
...
可选参数为MAILUSER和MAILPASSWORD，如果邮件服务器发送电子邮件需要认证。此配置为DEPRECATED。从privacyIDEA 2.10开始，您可以使用系统范围的SMTP server configuration。 配置如下所示：
