...
```
ServerTokens Prod  #将ServerTokens一行修改为Prod禁止显示版本
```
...
```
URL = https://pi.x.com/validate/check  #将127.0.0.1替换为前面设置的虚拟主机域名pi.x.com
```

同样将`/usr/share/privacyidea/freeradius/privacyidea_radius.pm`和`/usr/lib/python2.7/dist-packages/authmodules/FreeRADIUS/privacyidea_radius.pm`（这个文件不知道是不是必须）中的`https://127.0.0.1...`修改为`https://pi.x.com...`。

重启系统后测试是否能成功认证（先在网页上注册HOTP令牌）：
